...
import (
	...
	"github.com/jwhited/wgsd/vpplink/binapi/vppapi/interface_types"
	"github.com/jwhited/wgsd/vpplink/binapi/vppapi/ip_types"
	"github.com/jwhited/wgsd/vpplink/binapi/vppapi/wireguard"
	"github.com/jwhited/wgsd/vpplink/types"
	"github.com/pkg/errors"
)

func (v *VppLink) FindWireguardTunnel(swIfIndex uint32, showPrivateKey bool) (*types.WireguardTunnel, error) {
	tunnels, err := v.ListWireguardTunnels(showPrivateKey)
	if err != nil {
		return nil, err
	}
	for _, tunnel := range tunnels {
		if tunnel.SwIfIndex == swIfIndex {
			return tunnel, nil
			break
		}
	}
	return nil, errors.Errorf("Didnt find Wireguard tunnel [%d]", swIfIndex)
}

func (v *VppLink) ListWireguardTunnels(showPrivateKey bool) ([]*types.WireguardTunnel, error) {
	v.lock.Lock()
	defer v.lock.Unlock()
...
func (v *VppLink) ListWireguardPeers(swIfIndex uint32) ([]*types.WireguardPeer, error) {
	v.lock.Lock()
	defer v.lock.Unlock()
	...
	for {
		...
		if swIfIndex == uint32(response.Peer.SwIfIndex) || swIfIndex == ^uint32(0) {
